...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    const source = `${item?.nama ?? ''} ${item?.satuan ?? ''} ${item?.deskripsi ?? ''}`.toLowerCase()
...
const loadItems = async () => {
  ...
  try {
    const data = await realErpService.getFinancingProducts({ search: searchTerm.value.trim() })
    items.value = Array.isArray(data) ? data : []
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat produk expense.'
    items.value = []
  } finally {
    loading.value = false
  }
...
const goToCreate = () => {
  router.push('/real/produk-biaya/new')
}

const goToEdit = (id) => {
  router.push(`/real/produk-biaya/${id}/edit`)
}

const deleteItem = async (item) => {
  const ok = window.confirm(`Hapus produk expense ${item.nama}?`)
  if (!ok) return

  deletingId.value = item.id
  try {
    await realErpService.deleteFinancingProduct(item.id)
    toast.success('Produk expense berhasil dihapus.')
    await loadItems()
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Gagal menghapus produk expense.'
    toast.error(message)
  } finally {
    deletingId.value = ''
  }
...
<template>
  <section class="space-y-6">
    <SectionHeader
      eyebrow="Master Data"
      title="List Produk Expense"
      description="Kelola produk pembiayaan yang dipakai pada transaksi expense."
    />
    ...
    <PageState
      v-else-if="error"
      variant="error"
      title="Produk expense belum berhasil dimuat"
      :description="error"
    ...
    <PageState
      ...
      title="Produk expense belum tersedia"
      description="Tambahkan produk baru untuk mulai mencatat pembiayaan."
