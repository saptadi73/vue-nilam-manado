...
const normalizeCoordinateRows = (rawCoordinates) => {
...
const estimatedPolygonAreaM2 = computed(() => calculatePolygonAreaM2(validPolygonPoints.value))
const inputLuasM2 = computed(() => {
  const parsed = Number(form.value.luas)
  return Number.isFinite(parsed) ? parsed : null
})
const polygonAreaDiffM2 = computed(() => {
  if (!Number.isFinite(estimatedPolygonAreaM2.value) || !Number.isFinite(inputLuasM2.value)) return null
  return estimatedPolygonAreaM2.value - inputLuasM2.value
})
...
const loadLandDetail = async () => {
  ...
  let coordinatePayload = resolveLandCoordinatePayload(land)

  if (coordinatePayload.length < 3 && land?.pemilik_id) {
    try {
      const ownerLands = await realErpService.getLands({ pemilik_id: land.pemilik_id })
      const matchedLand = Array.isArray(ownerLands)
        ? ownerLands.find((item) => String(item?.id ?? '') === String(land?.id ?? props.id))
        : null
      const fallbackPayload = resolveLandCoordinatePayload(matchedLand)
      if (fallbackPayload.length) {
        coordinatePayload = fallbackPayload
      }
    } catch {
      // Keep using detail payload if fallback query fails.
    }
  }

  const coordinates = normalizeCoordinateRows(coordinatePayload).map((point) => ({
...
watch(
  () => [loading.value, showMap.value],
  async ([isLoading, isMapVisible]) => {
    if (isLoading) return
    if (!isMapVisible) {
      destroyMap()
      return
      ...
      <div class="mt-5 space-y-3">
        ...
        <div class="space-y-2">
          ...
          <div v-if="showMap" class="mt-2 overflow-hidden rounded-xl border border-emerald-500/30">
            ...
            <div class="border-t border-emerald-500/20 bg-emerald-950/20 px-3 py-2 text-xs text-emerald-100/85">
              <p>Estimasi luas polygon: {{ formatAreaM2(estimatedPolygonAreaM2) }} M²</p>
              <p v-if="polygonAreaDiffM2 !== null" :class="Math.abs(polygonAreaDiffM2) < 50 ? 'text-emerald-200/80' : 'text-amber-200/90'">
                Selisih vs luas input: {{ polygonAreaDiffM2 >= 0 ? '+' : '' }}{{ formatAreaM2(polygonAreaDiffM2) }} M²
              </p>
            </div>
